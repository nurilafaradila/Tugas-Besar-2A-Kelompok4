    ...
    if (str == "") {
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                document.getElementById("pesan").value = xmlhttp.responseText;
            }
        };
		var t = document.getElementById("nohp");
		var selectedText = t.options[t.selectedIndex].text;
		var tre=selectedText.substring(13,20);
        xmlhttp.open("GET","getipk.php?q="+str+"&c="+tre,true);
        xmlhttp.send();
    }
}
</script>
						...
						?>
                            ...
                            <form role="form" action="kirim_nilai.php?op=send" method="POST">
                                        <div class="form-group">
											 <?php
                                            echo "<tr><label>No Handphone </label>  <td>
											  <select class='form-control' name='nohp' id='nohp'>
												<option value=0 selected>- Pilih No HP -</option>";
												$tampil=mysql_query("SELECT * FROM t_orangtua_mhs ORDER BY npm");
												while($r=mysql_fetch_array($tampil)){
												  echo "<option value=$r[no_hp]>$r[no_hp]-$r[npm]</option>";
												}
										echo "</select></td></tr>";
										?>   
                                        </div>                                 
